...
class Controller(Node):
    def __init__(self):
        ...
        self.param_q_pp = self.get_parameter('q_pp').value
        self.param_m_pp = self.get_parameter('m_pp').value
    ...
    def control_loop(self):
        ...
        # If there are no waypoints, publish zero speed and steer to STOP
            ...
            self.get_logger().error('[PP Controller]: Received no waypoints. STOPPING!!!')
    ...
    def get_actuation(self, lookahead_point):
        """
        Returns the steering angle required to navigate to the lookahead point.
        
        Uses the pure pursuit algorithm to calculate the steering angle.
        
        Args:
            lookahead_point: Lookahead point in the map frame
            
        Returns:
            float: The steering angle in radians
        """
        # Convert the lookahead point from map frame to car frame
        lookahead_point_car_frame = self.map_to_baselink(lookahead_point)
        
        # Calculate the steering angle using pure pursuit
        y = lookahead_point_car_frame[1]
        steering_angle = np.arctan2(2 * self.wheelbase * y, 
                                    lookahead_point_car_frame[0]**2 + lookahead_point_car_frame[1]**2)
        
        return steering_angle

    def map_to_baselink(self, point_map):
        """
        Transform a point from the map frame to the base_link frame.
        
        Args:
            point_map: Point in the map frame [x, y]
            
        Returns:
            numpy.ndarray: Point in the base_link frame [x, y]
        """
        # Get the current position of the car
        x_car, y_car, theta_car = self.position
        
        # Translation
        x_translated = point_map[0] - x_car
        y_translated = point_map[1] - y_car
        
        # Rotation
        x_rotated = x_translated * np.cos(-theta_car) - y_translated * np.sin(-theta_car)
        y_rotated = x_translated * np.sin(-theta_car) + y_translated * np.cos(-theta_car)
